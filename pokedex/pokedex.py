...
class Pokedex():
    ...
    def play():
        with open("pokedex/pokedex.json", "r") as file:  # Lien avec le fichier .json
            data = json.load(file)
        ...
        # Fonction pour dessiner les boutons et afficher quand la souris va dessus

        def dessiner_boutons(fenetre, bouton_precedent_hover, bouton_suivant_hover):
            if bouton_precedent_hover:
                pygame.draw.rect(
                    fenetre, (100, 100, 100, 200), bouton_precedent)
            else:
            ...
            fenetre.blit(text_precedent, (bouton_precedent.x +
                                          10, bouton_precedent.y + 10))
            fenetre.blit(text_suivant, (bouton_suivant.x +
                         20, bouton_suivant.y + 10))

        # Fonction pour la couleur du font selon le type

        def afficher_pokemon(fenetre, pokemon):

            couleur_fond1 = couleurtype(pokemon['types'][0])
            couleur_fond2 = couleurtype(pokemon['types'][1]) if len(
                pokemon['types']) > 1 else couleur_fond1

            fenetre.fill(
                couleur_fond1, (0, 0, largeur_fenetre // 2, hauteur_fenetre))
            fenetre.fill(couleur_fond2, (largeur_fenetre // 2, 0,
                                         largeur_fenetre // 2, hauteur_fenetre))
        ...
        # Fiche du pokemon affiché
            stats = [
                ...
                f"Move: {', '.join([attaque['nom']
                                   for attaque in pokemon['Move']])}"
            ]
        ...
        while en_cours:
                ...
                if event.type == pygame.QUIT:
                    ...
                    if event.button == 1:
                        if bouton_precedent.collidepoint(event.pos):
                            index_pokemon_actuel = (
                                index_pokemon_actuel - 1) % len(data["pokedex"])
                        elif bouton_suivant.collidepoint(event.pos):
                            index_pokemon_actuel = (
                                index_pokemon_actuel + 1) % len(data["pokedex"])
                elif event.type == pygame.MOUSEMOTION:
                    bouton_precedent_hover = bouton_precedent.collidepoint(
                        event.pos)
                    bouton_suivant_hover = bouton_suivant.collidepoint(
                        event.pos)
            ...
            dessiner_boutons(fenetre, bouton_precedent_hover,
                             bouton_suivant_hover)

        # Chargement de l'image
            image = pygame.image.load('pokedex/pokedex.png')
